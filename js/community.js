// ============================================
// STUDY AURA — COMMUNITY (v2)
// Features: inline comments preview, emoji reactions,
//           report system, role badges, moderation
// ============================================
...
// ---- Render posts with inline comment preview ----
async function loadPosts() {
  ...
  if (posts.length === 0) {
    list.innerHTML = `<div class="empty-state">
      <svg width="40" height="40" style="color:var(--text3);margin-bottom:.5rem"><use href="#ic-users"/></svg>
  ...
  list.innerHTML = posts.map(post => {
    const allPostComments = commentMap[post.id] || [];
    const previewComments = allPostComments.slice(-2);
    ...
    <div class="post-body">
      <div class="post-header">
        ...
        <div class="post-header-info">
          ...
          <div class="post-time">${escHtml(profile.class || '')}${profile.class && profile.target_year ? ' · ' : ''}${escHtml(profile.target_year ? 'Target ' + profile.target_year : '')} · ${timeAgo(post.created_at)}</div>
        </div>
      </div>
      ...
      <div class="post-actions-row">
        ...
        <button class="post-report-btn" onclick="openReportModal('post', '${post.id}', null)">
          🚩 Report
        </button>
...
// ---- Post submit with mute check ----
document.getElementById('post-btn').addEventListener('click', async () => {
  if (!currentUser) return;
  if (currentProfile && currentProfile.muted_until && new Date(currentProfile.muted_until) > new Date()) {
    const muteEnd = new Date(currentProfile.muted_until).toLocaleTimeString();
    return showToast('🔇 You are muted until ' + muteEnd);
  }
  ...
  btn.disabled = false;
  if (error) return showToast('Error posting. Try again.');
  document.getElementById('post-content').value = '';
  showToast('✅ Target posted!');
  loadPosts();
});
...
async function openComments(postId, event) {
  ...
  document.getElementById('modal-post-content').innerHTML = '<div class="loading-text">Loading...</div>';
  document.getElementById('comments-list').innerHTML = '<div class="loading-text">Loading comments...</div>';
    ...
    document.getElementById('modal-post-content').innerHTML = `
      <div class="modal-post">
        <div class="modal-post-header">
          ...
          <div>
            <span class="post-author-name">${escHtml(p.name || 'Anonymous')}${roleBadge(p.role)}</span>
...
function renderCommentsInModal(comments) {
  ...
  if (!comments || comments.length === 0) {
    list.innerHTML = `<div class="empty-state" style="padding:2rem">
      <svg width="28" height="28" style="color:var(--text3);margin-bottom:.4rem"><use href="#ic-chat"/></svg>
  ...
  list.innerHTML = comments.map(c => {
    ...
    return `<div class="comment-item" id="comment-${c.id}">
      ...
      <div class="comment-content">
        <div class="comment-author">${escHtml(name)}${roleBadge(c.profiles ? c.profiles.role : '')} <span class="comment-time">${timeAgo(c.created_at)}</span></div>
        <div class="comment-text">${escHtml(c.content)}</div>
        <div class="comment-actions">
          <button class="comment-action-btn" onclick="openReportModal('comment', null, '${c.id}')">🚩 Report</button>
          ${canModerate() ? `<button class="comment-action-btn mod-btn" onclick="modDeleteComment('${c.id}')">🗑️ Delete</button>` : ''}
          ${canModerate() && c.profiles && c.profiles.role !== 'admin' ? `<button class="comment-action-btn mod-btn" onclick="openMuteModal('${c.profiles.id}', '${escHtml(name)}')">🔇 Mute</button>` : ''}
        </div>
      </div>
    </div>`;
  }).join('');
...
document.getElementById('submit-comment-btn').addEventListener('click', async () => {
  ...
  btn.disabled = true;
  const { error } = await db.from('comments').insert({ post_id: activePostId, user_id: currentUser.id, content });
  btn.disabled = false;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', async () => {
      ...
      if (!reason) return showToast('Please describe the issue');
...
async function applyMute(minutes) {
  ...
  const { error } = await db.from('profiles').update({ muted_until: muteUntil }).eq('id', muteTargetUserId);
  if (error) return showToast('Error: ' + error.message);
  closeMuteModal();
  const label = minutes < 60 ? minutes + ' minutes' : (minutes / 60) + ' hour(s)';
  showToast('🔇 User muted for ' + label);
}
